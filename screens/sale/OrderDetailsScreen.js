...
export default function OrderDetailsScreen(props) {
  ...
  const url = `${BASE_URL}/api/sale.order/` + id;
  ...
  const renderOrderLine = ({ item: orderLine }) => {
    return (
      <View
        style={{
          ...styles.tableRow,
      ...
      >
        ...
        <Text style={[styles.columnRowTxt, { textAlign: "right" }]}>
          {parseFloat(orderLine.price_unit).toFixed(3)}
        </Text>
        <Text style={[styles.columnRowTxt, { textAlign: "right" }]}>
          {parseFloat(orderLine.price_subtotal).toFixed(3)}
        </Text>
      </View>
    );
  };
      ...
      <View style={styles.headerInfoCustomer}>
        ...
        <View style={{ flexDirection: "row" }}>
          <View style={styles.headerInfoOrderDate}>
            <Text style={styles.headerInfoSaleTitle}>Date commande</Text>
            <Text style={styles.headerInfoSaleText}>{order.date_order}</Text>
          </View>
        ...
        <Text style={styles.headerInfoNameOrder}>{order.name}</Text>
        ...
        >
          <View style={styles.headerInfoTotal}>
            ...
            <Text style={styles.headerInfoSaleText}>
              {parseFloat(order.amount_tax).toFixed(3)}{" "}
          ...
          <View style={styles.headerInfoTotal}>
            ...
            <Text style={styles.headerInfoSaleText}>
              {parseFloat(order.amount_total - order.amount_tax).toFixed(3)}{" "}
              {order?.currency_id?.name}
            </Text>
          </View>
          <View style={styles.headerInfoTotal}>
            ...
            <Text style={styles.headerInfoSaleText}>
              {parseFloat(order.amount_total).toFixed(3)}{" "}
              {order?.currency_id?.name}
            </Text>
          </View>
        </View>
